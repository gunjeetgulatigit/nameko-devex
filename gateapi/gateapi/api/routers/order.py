...
def get_order(order_id: int, nameko: NamekoRPCPoolDep = Depends(nameko_rpc)):
    try:
        return _get_order(order_id, nameko)
...
def _get_order(order_id, nameko):
    ...
    image_root = "http://www.example.com/airship/images"
...
def create_order(request: schemas.CreateOrder, nameko: NamekoRPCPoolDep = Depends(nameko_rpc)):
    id_ =  _create_order(request.dict(), nameko)
    return {
        'id': id_
    }

def _create_order(order_data, nameko):
